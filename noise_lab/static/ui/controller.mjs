...
import {createSoundEffects} from './sound-effects.mjs';
...
async function refreshGeneration() {
  ...
  try {
    ...
    soundEffects?.setAvailability(result.sound_effects === true, result.sound_effects_status);
    ...
    aiReady = false;
    soundEffects?.setAvailability(false);
    await patchLibrary?.setAvailability(false);
...
soundEffects = createSoundEffects({
  getRevision: () => `${generation}:${recipeRevision}`,
  prepareDownload,
  async loadAudio(bytes, accept) {
    if (loading || faulted) throw new Error('Audio is busy.');
    const token = generation;
    loading = true; cancelGeneration(false); renderTransport();
    try {
      const audio = await ensureEngine();
      if (!accept()) throw new Error('Newer work retained.');
      const info = await audio.loadGenerated(bytes, accept);
      sourceId = null; recipeRevision++;
      updateSource(info);
      $('source-description').textContent = 'Generated by ElevenLabs from your description. Processing happens on this device.';
      $('source-badge').textContent = 'Generated audio';
      applyAudio();
    } finally { if (token === generation) {loading = false; renderTransport();} }
  },
});
