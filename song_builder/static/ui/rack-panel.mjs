...
export function renderRackPanel(root,{track,blocked,protectedNames,allowed,preview,apply,audition=false}){
  ...
  if(hasTrack)for(const [label,enabled]of [['Original',false],['Processed',true]])modes.append(action(label,()=>apply({...settings,enabled}),settings.enabled===enabled));
  ...
  for(const [key,label,min,max,detail]of controls){
    ...
    const input=document.createElement('input');input.type='range';input.min=min;input.max=max;input.step=1;input.value=settings[key];input.dataset.control=key;input.dataset.rackAssigned='';input.setAttribute('aria-label',`${label} rack control`);input.disabled=blocked||!settings.enabled;
    ...
    cancellations.push(bindRackSlider(input,{read:()=>({...settings}),allowed:()=>allowed()&&hasTrack&&settings.enabled,preview,apply,display}));
