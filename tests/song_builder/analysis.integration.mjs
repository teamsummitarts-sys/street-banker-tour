...
test('real Flask + DOM: upload stays unassigned, choose local, review and persist editable blueprint',async()=>{
 ...
 try{
  ...
  dom=new JSDOM(await page.text(),{url:origin+'/song-builder/analyze'});
  Object.assign(globalThis,{document:dom.window.document,window:dom.window,Option:dom.window.Option});
  ...
  Object.defineProperty(d.getElementById('upload'),'files',{value:[new File([wav()],'original.wav',{type:'audio/wav'})]});
 ...
 }finally{server.kill();dom?.window.close();globalThis.fetch=nativeFetch;}
});
